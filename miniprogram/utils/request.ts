...
type AfterRequestSucFn = (
	res:
		| WechatMiniprogram.RequestSuccessCallbackResult<RequestRes<any>>
		| RequestRes<any>
) =>
	| Promise<
			| WechatMiniprogram.RequestSuccessCallbackResult<RequestRes<any>>
	...
	| WechatMiniprogram.RequestSuccessCallbackResult<RequestRes<any>>
	| RequestRes<any>;
type AfterRequestFailFn = (
	err: WechatMiniprogram.RequestFailCallbackErr
...
class Request {
	...
	async request<T>(config: RequestOptions) {
		...
		try {
			let res:
				| WechatMiniprogram.RequestSuccessCallbackResult<RequestRes<T>>
				| RequestRes<T> = await wxRequest<T>(wxRequestConfig);
